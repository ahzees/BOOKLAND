...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    """
    first_name = models.CharField(max_length=20, default=None, null=True, blank=True, verbose_name='Імʼя')
    last_name = models.CharField(max_length=20, default=None, null=True, blank=True, verbose_name='Прізвище')
    middle_name = models.CharField(max_length=20, default=None, null=True,blank=True, verbose_name='Друге імʼя')
    ...
    def __str__(self):
        ...
        """
        return f"id: {self.id},{self.first_name} {self.last_name}, email: {self.email}" \
            # f", 'created_at': {int(self.created_at.timestamp())}, 'updated_at': {int(self.updated_at.timestamp())}, 'role': {self.role}, 'is_active': {self.is_active}"  # 'password': '{self.password}', \
    ...
    def count_all_orders(self):
        ...
        x = Order.objects.filter(user_id=self.pk)
        return len(x)
