...
class Author(models.Model):
    ...
    """
    name = models.CharField(blank=True, max_length=20, verbose_name='Імʼя')
    ...
    def __str__(self):
        ...
        return f"\'id\': {self.pk} {self.name} " \
               f" {self.surname} {self.patronymic}"
    ...
    def get_all_0():
        lst = []
        for i in Author.get_all():
            if not len(i.books.all()):
                lst.append(i)
        return lst

    def get_absolute_url(self):
        return reverse('view_author', kwargs={'author_id': self.pk})
