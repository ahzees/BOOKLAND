...
class Book(models.Model):
    ...
    name = models.CharField(max_length=128, null=False, blank=False, verbose_name='Назва')
    description = models.CharField(max_length=256, unique=True, null=False, blank=False, verbose_name='Опис')
    count = models.IntegerField(default=10, null=False, verbose_name='Кількість екземплярів')
    id = models.AutoField(primary_key=True)
    year_of_publication = models.DateField(default=None, null=True, blank=True)
    date_of_issue_of_the_book = models.DateField(default=None, null=True, blank=True, editable=True)
    ...
    def create(name, description, count=10, authors=None):
        ...
        if len(name) > 128:
            return None
        book = Book()
        if not name or not description or not count:
            raise Exception
        book.name = name
    ...
    def get_absolute_url(self):
        return reverse('view', kwargs={'book_id': self.pk})
